...
# NDK路径，使用 Windows 路径格式
ANDROID_NDK_ROOT_WIN="C:/Users/pc/AppData/Local/Android/Sdk/ndk/26.1.10909125"
...
function build_android() {
    ...
    ./configure \
        ...
        --enable-protocol=udp \
        --enable-protocol=tcp \
        --extra-cflags="-O3 -fPIC -std=c11 -DANDROID_NDK -DNDEBUG" \
        --extra-cxxflags="-std=c++11" \
        --extra-ldflags="-lc -lm -ldl -llog -lz -Wl,--no-undefined -Wl,-z,noexecstack" || exit 1
    ...
    # 使用编译器创建动态库
    "$CC" -shared -fPIC -Wl,--whole-archive \
